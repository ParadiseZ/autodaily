...
fun templateMatch(
    ...
    picNameList?.forEach {
        ...
        if (!is720p) {
            ...
            Imgproc.matchTemplate(scaledOriginalMat, templateMat, result, Imgproc.TM_SQDIFF)
        }else{
            Imgproc.matchTemplate(originalMat, templateMat, result, Imgproc.TM_SQDIFF)
        }
        // 获取最佳匹配位置
        ...
        if (res.minVal < similarityThreshold) {
            if (matchSave) {
                ...
                if (is720p) {
                    ...
                    sai.point = Point(
                        (res.minLoc.x * originalMat.cols() / scaledOriginalWidth).roundToInt()
                            .toFloat(),
                        (res.minLoc.y * originalMat.rows() / scaledOriginalHeight).roundToInt()
                            .toFloat()
                    )
                }
            }else{
